...
def _money_or_na(value: float | None, unit: str | None) -> str:
    """통화에 맞춰 적는다. 값이 없으면 종전과 같이 'N/A'."""
    return "N/A" if value is None else format_money(value, unit or "원")
...
def make_munger_facts_bundle(analysis: dict[str, any]) -> dict[str, any]:
    ...
    facts = {
        ...
        "정규화 FCF": _money_or_na(_r(val.get("normalized_fcf"), 0), analysis.get("money_unit")),
        "적정가 추정치": _money_or_na(_r(ivr.get("reasonable"), 0), analysis.get("money_unit")),
