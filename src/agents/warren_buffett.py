...
def build_buffett_evidence_summary(analysis_data: dict[str, any]) -> dict[str, any]:
    ...
    return {
        ...
        "formatted_evidence": {
            ...
            "부채비율 (debt-to-equity)": format_leverage_ratio(metrics.get("debt_to_equity")),
        ...
        "valuation_summary": {
            "소유자 이익 (owner earnings)": format_money(valuation.get("owner_earnings"), currency),
            "내재가치 (intrinsic value)": format_money(valuation.get("intrinsic_value"), currency),
            "시가총액 (market cap)": format_money(analysis_data.get("market_cap"), currency),
            "안전마진 (margin of safety)": format_percent(margin_of_safety),
