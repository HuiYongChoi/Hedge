...
RATIO_SCALE_REQUIREMENT = (
    ...
    "Use Korean first with English in parentheses for important terms, for example 안전마진 (margin of safety) and 유동비율 (current ratio). "
...
- 나쁜 부채: ROIC가 낮거나 악화되고, interest_coverage가 취약하며, operating_cash_flow 또는 free_cash_flow가 부진해서 영업 손실/운영자금 부족을 차입으로 메우는 정황이 제공 지표에서 확인되는 경우.

한계 처리:
- 산업/비즈니스 모델, 수주 잔고, 리츠/인프라형 안정 현금흐름, 부채 사용 목적, M&A 시너지, 확실한 수요 증가, 본업 외 확장 여부가 입력 데이터에 없으면 추정하지 마십시오.
- 위 항목이 결론에 중요하지만 검증되지 않았으면 ### 검토 필요 항목에 원문 공시 또는 사업보고서에서 확인할 항목으로 작성하십시오.
- Never invent numbers, debt purposes, acquisition synergies, backlog quality, or industry-specific leverage tolerance.
"""
...
### 검토 필요 항목
- 정량 지표만으로 단정할 수 없는 산업/비즈니스 모델, 부채 사용 목적, M&A/설비투자 수요 확실성, 본업 외 확장 여부를 원문 공시에서 확인할 항목으로 작성.
"""
