...
const RUBRIC = [
  ...
  // 2026-09-04 실측: 한 보고서에 아래 네 종류가 동시에 나왔다. 목록에 없는
  // 지표가 생길 때마다 같은 일이 반복돼 왔으므로 규칙으로 못 박는다.
  { id: 'dupnumber',   w: 10, label: '같은 수를 두 번 표기 ("43%(약 42.75%)")',
    bad: /\d+(?:\.\d+)?\s*%\s*\(\s*약?\s*\d+(?:\.\d+)?\s*%\s*\)|\b0?\.\d+\s*\(\s*약?\s*\d+(?:\.\d+)?\s*%\s*\)/g },
  { id: 'currencycode', w: 8, label: '통화 코드 노출 ("62,656.6 KRW")',
    bad: /\b\d[\d,]*(?:\.\d+)?\s*(?:KRW|USD\b(?!\/)|JPY)\b/g },
  { id: 'echoparen',   w: 8,  label: '괄호가 앞말을 영어로 되풀이 ("선행 PER(선행 P/E)")',
    bad: /선행\s*PER\s*\(\s*선행\s*P\/?E\s*\)/gi },
  { id: 'stuckperiod', w: 8,  label: '마침표 뒤 공백 없이 문장이 붙음 ("가능.다만", "있음).컨센서스")',
    bad: /[가-힣]\.[가-힣]|\d\.[가-힣]|[)）]\.[가-힣]/gu },
  { id: 'methodlist', w: 10, label: '분석 틀 목차가 본문에 남음 ("FCFF DCF + 안전마진 + 상대가치 체크")',
    bad: /DCF\s*\+\s*안전마진\s*\+\s*상대가치/gu },
  { id: 'dupword',     w: 8,  label: '같은 낱말이 연달아 ("근거 근거는")',
    bad: /\b([가-힣]{2,4})\s+\1(?=[은는이가을를의로])/gu },
  { id: 'badparticle', w: 8,  label: '받침에 안 맞는 조사 ("판정가", "위험로 명시")',
    bad: /\b(?:판정|점검|근거|결론|수준|비율|구간)가(?=\s|$|[,.])|\b위험로\b/gu },
