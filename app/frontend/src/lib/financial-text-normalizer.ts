...
function tidyKoreanProse(text: string): string {
  return text
    ...
    .replace(/(\d)\.(?=[가-힣])/gu, '$1. ')
    // 괄호로 끝난 문장도 같다: "바꿀 수 있음).컨센서스가"(실측: MCD).
    .replace(/([)）])\.(?=[가-힣])/gu, '$1. ')
    // 분석 틀의 목차("가치(Value): FCFF DCF + 안전마진 + 상대가치 체크")가 실제 내용 앞에
    // 붙어 나온다(실측: MCD "…상대가치 체크 FCFF DCF: 내재가치 …"). 내용만 있는
    // 카드는 isFrameworkLabelOnly 가 걸러 주지만, 뒤에 내용이 붙으면 그대로 남았다.
    .replace(
      /(?:가치\s*\(\s*(?:value|가치)\s*\)\s*[:：]\s*)?(?:FCFF\s*)?DCF\s*\+\s*안전마진\s*\+\s*상대가치\s*(?:체크|점검)\s*[:：]?\s*(?=\S)/giu,
      '',
    )
