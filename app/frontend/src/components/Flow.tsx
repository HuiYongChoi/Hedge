...
export function Flow({ className = '' }: FlowProps) {
  ...
  // Persist runtime data (agent messages, reports, and output node data) after SSE updates.
  useEffect(() => {
    if (!isInitialized || !currentFlowId) return;

    const flowId = currentFlowId.toString();
    const nodeContextData = exportNodeContextData(flowId);
    const hasRuntimeData = Object.keys(nodeContextData.agentNodeData || {}).length > 0
      || Boolean(nodeContextData.outputNodeData);
    ...
    return () => clearTimeout(timeoutId);
  }, [autoSave, currentFlowId, exportNodeContextData, isInitialized]);
